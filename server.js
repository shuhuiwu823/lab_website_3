...
app.get('/team_stats', function (req, res) {
	...
	var lose = 'SELECT COUNT(*) FROM football_games WHERE home_score < visitor_score'
...
app.get('/player_info/post', function(req, res) {
	var playerIdx = req.query.player_choice;
	var players = 'SELECT id, name FROM football_players;';
	var queryB = 'SELECT * FROM football_players WHERE id = ' + playerIdx + ';';
	var queryC = 'SELECT COUNT(*) FROM football_games WHERE ' + playerIdx + ' = any(players);';
	...
	db.task('get-everything', task => {
		...
		.then(data => {
			res.render('pages/player_info', {
				my_title: "Player Info Page",
				player_id_name_list: data[0],
				player_info_chosen: data[1][0],
				player_played_chosen: data[2][0].count
			})
		})
		.catch(error => {
			// display error message in case an error
